...
import {
    adminGetPaymentMethodByIdController,
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return await adminGetPaymentMethodByIdController(req, { params });
    } catch (error) {
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return await updatePaymentMethodController(req as any, { params });
    } catch (error) {
        if (error instanceof CustomError) return NextResponse.json({ message: error.message }, { status: error.statusCode });
        return NextResponse.json({ message: 'Internal Server Error' }, { status: 500 });
    }
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return await deletePaymentMethodController(req as any, { params });
    } catch (error) {
        if (error instanceof CustomError) return NextResponse.json({ message: error.message }, { status: error.statusCode });
        return NextResponse.json({ message: 'Internal Server Error' }, { status: 500 });
    }
