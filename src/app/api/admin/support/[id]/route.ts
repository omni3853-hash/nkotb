...
import {
    adminGetSupportTicketByIdController,
    adminUpdateSupportTicketStatusController,
} from "@/lib/controllers/support-ticket.controller";
...
export async function GET(req: AuthRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return await adminGetSupportTicketByIdController(req, { params });
    } catch (e: any) {
...
export async function PATCH(req: AuthRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const middlewareResponse = await isLoggedIn(req);
        if (middlewareResponse.status !== 200) return middlewareResponse;
        return await adminUpdateSupportTicketStatusController(req, { params });
    } catch (e: any) {
        if (e instanceof CustomError) {
            return NextResponse.json({ message: e.message }, { status: e.statusCode });
        }
