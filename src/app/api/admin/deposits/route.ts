...
import { adminCreateDepositController, adminListDepositsController } from "@/lib/controllers/deposit.controller";
...
export async function GET(req: NextRequest) {
    try {
        ...
        return await adminListDepositsController(req as any);
    } catch (e) {
...
export async function POST(req: NextRequest) {
    try {
        ...
        const middlewareResponse = await isLoggedIn(req, true);
        if (middlewareResponse.status !== 200) return middlewareResponse;
        return await adminCreateDepositController(req as any);
    } catch (e) {
        if (e instanceof CustomError) return NextResponse.json({ message: e.message }, { status: e.statusCode });
        return NextResponse.json({ message: "Internal Server Error" }, { status: 500 });
    }
