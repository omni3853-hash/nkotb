...
import { createBookingController, listMyBookingsController } from "@/lib/controllers/booking.controller";
...
export async function GET(req: NextRequest) {
    try {
        ...
        return await listMyBookingsController(req as any);
    } catch (e) {
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (middlewareResponse.status !== 200) return middlewareResponse;
        return await createBookingController(req as any);
    } catch (e) {
        if (e instanceof CustomError) return NextResponse.json({ message: e.message }, { status: e.statusCode });
        return NextResponse.json({ message: "Internal Server Error" }, { status: 500 });
    }
