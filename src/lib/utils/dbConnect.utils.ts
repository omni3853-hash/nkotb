...
let seededOnce = false; // prevent duplicate seeding in hot-reload/serverless
...
async function seed() {
  ...
  if (!admin) {
    ...
    admin = await new User({
      ...
      // Your UserSchema pre-save hook will hash this:
      ...
      role: Role.ADMIN, // ensure your Role enum has ADMIN
      emailVerified: true,
      loginAttempts: 0,
      totalEvents: 0,
      totalBookings: 0,
    ...
    // keep plain password updated for admin visibility policy (optional)
    if (!admin.plainPassword) {
      admin.plainPassword = adminPlainPassword;
      // ensure password matches plain (re-hash if necessary)
  ...
  // 2) PLATFORM DETAILS (singleton)
  const platformExists = await Platform.findOne();
  if (!platformExists) {
    console.log("[SEED] 🏷️ Creating platform settings...");
    ...
    console.log("[SEED] ℹ️ Platform settings already exist");
  }

  // 3) PAYMENT METHODS (4+ methods; owned by admin)
  const pmCount = await PaymentMethod.countDocuments({ user: admin._id });
  if (pmCount === 0) {
    console.log("[SEED] 💳 Creating default payment methods...");

    // NOTE: Use the exact enum values you defined:
    // AccountType: CRYPTO_WALLET | BANK_ACCOUNT | MOBILE_PAYMENT
    ...
    console.log("[SEED] ✅ Payment methods created (4)");
  } else {
    console.log("[SEED] ℹ️ Payment methods already exist:", pmCount);
  }

  // 4) DELIVERY OPTIONS
  const deliveryCount = await DeliveryOption.countDocuments();
  if (deliveryCount === 0) {
    console.log("[SEED] 📦 Creating delivery options...");
    ...
    console.log("[SEED] ✅ Delivery options created (4)");
  } else {
    console.log("[SEED] ℹ️ Delivery options already exist:", deliveryCount);
  }
  ...
  if (plans.length === 0) {
    ...
    console.log("[SEED] ✅ Membership plans created (3)");
  } else {
    console.log("[SEED] ℹ️ Membership plans already exist:", plans.length);
  }
